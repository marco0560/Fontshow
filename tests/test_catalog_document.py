...
def test_generate_document_labels_single_script_extended_specimen(
    monkeypatch, tmp_path
):
    """
    Ensure extended catalogs label single-script specimen rows.
    ...
    None
    """
    font_path = tmp_path / "DroidSansThai.ttf"
    font_path.write_text("", encoding="utf-8")
    ...
    monkeypatch.setattr(
        ...
        "SCRIPT_INFO",
        {
            ScriptISO("THAI"): {"rtl": False, "specimen": "Thai sample"},
        },
    )

    def render_stub(font, safe_specimen, script0_iso, fullpath, catalog_detail=None):
        """
        Return a deterministic marker for a single rendered script.
        ...
        tuple[str, str]
            Fake LaTeX render block and plain option string.
        """
        _ = font, fullpath, catalog_detail
        return f"<{script0_iso}|{safe_specimen}>", f"Path={script0_iso}"
        ...
        [
            {
                ...
                "style": "Regular",
                "script": "thai",
                "specimen_text": "Original Thai",
                "inference": {"scripts": ["thai"], "languages": ["th"]},
                "coverage": {"scripts": ["thai"]},
            },
        ],
        catalog_detail="extended",
    )

    assert "{\\footnotesize\\ttfamily FILE  : DroidSansThai.ttf}" in latex
    assert "{\\footnotesize\\ttfamily SPEC  : THAI}" in latex
    assert (
        "\\begin{minipage}[t]{0.18\\linewidth}\\raggedright "
        "{\\footnotesize\\ttfamily SPEC  : THAI}\\end{minipage}" in latex
    )
    assert (
        "\\begin{minipage}[t]{0.79\\linewidth}<THAI|Original Thai>"
        "\\end{minipage}\\par" in latex
    )
